...
class ForeignKeystoreManagementScreen(WAScreenBase):
    ...
    def list_foreign_keystores(self, display_toast=True):
        ...
        for (index, (keystore_uid, metadata)) in enumerate(sorted(keystore_metadata.items()), start=1):
            ...
            selected = str(keystore_uid) in self.selected_keystore_uids
            data = dict(
                unique_identifier = keystore_uid,
            ...
            # For some reason, authenticator_entry.selected binding doesn't work here
            authenticator_entry.ids.selection_checkbox.active = selected
            print(">>>>>>>> SELECTED", authenticator_entry.selected)
