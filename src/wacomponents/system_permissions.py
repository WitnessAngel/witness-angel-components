...
def request_multiple_permissions(permissions: List[str]) -> List[bool]:
    ...
    if IS_ANDROID:
        ...
        permissions_qualified_names = [getattr(Permission, permission) for permission in permissions]
        request_permissions(permissions_qualified_names)  # Might freeze app while showing user a popup
...
def __OBSOLETE_request_external_storage_dirs_access():
    # FIXME rename to request_external_storage_dir_access()?
